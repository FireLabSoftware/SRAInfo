...
# FireLab_SRAInfoFinder version ah02 08-20-24
...
## -  Many thanks to UriNeri for catching a bug in exception handling     
## End Help        
...
def main():
    ...
    if OutputFileName1 == 'default':
        OutputFileName1 = "SRAInfo_"+os.path.basename(InFileName1).split('.')[0]+'_'+vnow+'.tsv'
    t0 = time()
    ...
    for i1 in range(MaxLines1):
        ...
        if i1 == 0: lL1 = len(L1)
        Accession1 = L1[IDColumn1].split('_')[0]
    ...
    while OngoingProcesses1:
        for i,p1 in enumerate(OngoingProcesses1):            
            if not(p1.is_alive()):
                ...
                accession0,MyText0 = MyQueue1.get()
                ...
                SRAToMetaData1[accession0] = '\t'.join(ThisInfo)
        sleep(0.001)
    ...
    for i11 in range(MaxLines1):
        ...
        Accession11 = OutLine11[IDColumn1].strip().split('_')[0]
